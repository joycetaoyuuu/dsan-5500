...
class Experiment:
    ...
    def __confusionMatrix(self):
        ...
        for idx, val in enumerate(self.matrix): # go through predictions in each classifier
            ...
            print("Confusion Matrix for ",self.clf_dict.get(idx))
            for x in cm:
                print(x)
...
# Complexity Analysis
# Assume that training data has the size (m1*n), training label has the size (m1*1), 
# testing data has the size (m2*n), testing label has the size (m2*1),
# there are x classifiers available for experiment

# a. simple KNN Classifier test method:

# T(n) = (assigning values) + (outer loop: get each test data) + (inner loop: get distance for each train value) + (get the cloest n items)
#      =          5         +          n + n*m_1               +    n*m_2 + (n^2)*m_2 + (n^2)*m_2 + n*m_2        +   n*m_1 + n + n + n + n
#      = 2(m_2)n^2 + (5+2m_1+2m_2)n + 5 = O(n^2)

# S(n) = 5 (initial variables) + 1 (variable i for looping) + n (x_test_data, a list with length n) + 1 (variable j for looping) + 1 (variable label) + n (x_train_data, a list with length n) + 1 (distance) + 2n (table generated) + k (top k labels) + 1 (mode of the labels) + n (y_pred for each x_test) = (10+k) + 5n = O(n)

# b. Expreiment score method
# T(n) = (create empty df) + (outer loop: loop over all classifiers) + (innrer loop: calculating accuracy for each clf) + accuracy
#      =       1+n         +                  (n+1)x                 +                   (1+1)nx                        +  3x
#      = (3x+1)n + 4(x+1) = O(n)

# S(n) = 2x (for accuracy_datafreame) + n (actual variable saved in a list) + 2 (idx, val in loop) + n (pred variable in list) + 1 (num for count) + 1 (i as the variable in loop) + 2 (accuracy,clf_name as single variable)
#      =  2n + (6+2x) = O(n)

# c. Experiment confusion matrix method
# T(n) = (assigning values) + (building dict) + (outer for loop) + (inner for loop)
#      =      n+n+1         +      n          +  xn^2+xn         +  3xn
#      =  xn^2 + 4xn + 3n + 1 = O(n^2)
# S(n) = n (actual variable saved in a list) + n (labels saved in a set) + n (dictionary saving labels) + 2 (idx, val variable in the loop) + n^2 (n*n confusion matrix) + n (predicted variable in a list) + 2(x,y single variables)
#      = n^2 + 4n + 4 = O(n^2)
